...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-gradient-to-r from-purple-400 to-pink-500 rounded-xl p-6 text-white">
                    ...
                    <p class="text-lg font-bold mb-1">{{ $winner->product_name }}</p>
                    <p class="text-sm opacity-90 mb-2">Value: Rs. {{ number_format((float) $winner->product_price, 2) }}</p>
        ...
        <div class="mb-12">
            ...
            @if(empty($product_draws) || count($product_draws) === 0)
            <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-12 text-center">
                ...
                <a href="{{ url('cash-draws') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 relative">
                    ...
                    <div class="h-40 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden relative">
                        <img src="{{ get_product_image($draw->product_image) }}" alt="{{ $draw->product_name }}" class="w-full h-full object-contain p-3">
                    </div>
                    ...
                    <div class="p-4">
                        ...
                        <div class="text-center mb-3">
                            <div class="text-sm text-gray-500 line-through">
                                Price Rs. {{ number_format((float) $draw->product_price, 0) }}
                            </div>
                        </div>
                        ...
                        @else
                        <button onclick="joinProductDraw({{ $draw->id }}, {{ (float) $draw->entry_fee }})"
                            ...
                            Join - Rs. {{ number_format((float) $draw->entry_fee, 2) }}
                        </button>
                        @endif
                        @else
                        <a href="{{ url('login') }}"
                        ...
                        <div class="mt-3 text-xs text-gray-600 text-center">
                            {{ $draw->total_winners ?? 1 }} winner{{ ($draw->total_winners ?? 1) > 1 ? 's' : '' }} • {{ \Carbon\Carbon::parse($draw->draw_date)->format('M d') }}
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
            ...
            <div class="space-y-4">
                @foreach(collect($completed_draws)->take(5) as $draw)
                @php
                $draw = is_array($draw) ? (object) $draw : $draw;
                @endphp
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <div class="w-16 h-16 rounded-lg overflow-hidden">
                                <img src="{{ get_product_image($draw->product_image) }}" alt="{{ $draw->product_name }}" class="w-16 h-16 object-cover rounded-lg">
                            </div>
                            <div>
                                <h4 class="font-semibold text-gray-900">{{ $draw->title }}</h4>
                                <p class="text-sm text-gray-600">{{ $draw->product_name }} • Value: Rs. {{ number_format((float) $draw->product_price, 2) }}</p>
