...
<div class="space-y-6"></div>
    ...
    <div class="bg-gradient-to-r from-green-600 to-emerald-700 rounded-2xl shadow-lg p-8 text-white">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold">Edit Product Lucky Draw</h1>
     ...
     <!-- draw table -->    
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 py-6">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            @if ($errors->any())
                <div class="bg-red-50 border border-red-500 text-red-500 px-4 py-3 rounded relative" role="alert">
                    <ul>
            ...
            <form action="{{route('admin.productdraws.update', $productDraw->id)}}" method="POST" class="p-6 space-y-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" required
                            ...
                            placeholder="e.g., Win $1000 Cash Prize" value="{{old('title', $productDraw->title)}}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            placeholder="Brief description of the cash draw...">{{old('description', $productDraw->description)}}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="product_name" id="product_name" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            placeholder="e.g., iPhone 13" value="{{old('product_name', $productDraw->product_name)}}">
                        @error('product_name')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="product_price" id="product_price" step="0.01" min="1" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            placeholder="1000.00" value="{{old('product_price', $productDraw->product_price)}}">
                            @error('product_price')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="entry_fee" id="entry_fee" step="0.01" min="1" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            placeholder="1.00" value="{{old('entry_fee', $productDraw->entry_fee)}}">
                        @error('entry_fee')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="datetime-local" name="draw_date" id="draw_date" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            value="{{old('draw_date', date('Y-m-d\TH:i', strtotime($productDraw->draw_date)))}}">
                        @error('draw_date')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div id="productPreview" class="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl p-6 border-2 border-blue-200 shadow-lg">
                    <div class="text-center">
                        ...
                        <div id="previewImageContainer" class="w-48 h-48 mx-auto mb-6 bg-white rounded-xl shadow-md flex items-center justify-center overflow-hidden">
                            ...
                            <img id="previewImage" src="{{asset('storage/' . $productDraw->product_image)}}" alt="Product" class="w-full h-full object-cover hidden">
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updatePreviewImage() {
            ...
            if(image) {
                ...
                previewImage.src = "{{asset('storage/' . $productDraw->product_image)}}";
                previewImage.classList.remove('hidden');
