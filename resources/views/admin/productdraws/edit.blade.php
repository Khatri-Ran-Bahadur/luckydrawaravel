...
<div class="space-y-6">
     ...
     <div class="bg-gradient-to-r from-green-600 to-emerald-700 rounded-2xl shadow-lg p-8 text-white">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-green-100 mt-1">Edit a cash prize draw</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100">
        ...
        <form action="{{route('admin.cashdraws.update', $cashdraw->id)}}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" required
                        ...
                        placeholder="e.g., Win $1000 Cash Prize" value="{{old('title',$cashdraw->title)}}">
                    @error('title')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="prize_amount" id="prize_amount" step="0.01" min="1" required
                            ...
                            placeholder="1000.00" value="{{old('prize_amount',$cashdraw->prize_amount)}}">
                            @error('prize_amount')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="entry_fee" id="entry_fee" step="0.01" min="0.01" required
                            class="w-full pl-8 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                            placeholder="1.00" value="{{old('entry_fee',$cashdraw->entry_fee)}}">
                            @error('entry_fee')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    </div>
                ...
                <div>
                    ...
                    <input type="number" name="total_winners" id="total_winners" min="1" required
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                        placeholder="1" value="{{old('total_winners',$cashdraw->total_winners)}}">
                        @error('total_winners')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="draw_date" id="draw_date" required
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                        value="{{old('draw_date',$cashdraw->draw_date)}}">
                        @error('draw_date')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    placeholder="Brief description of the cash draw...">{{old('description',$cashdraw->description)}}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            </div>
            ...
            <div>
                ...
                <div class="space-y-3">
                    <div class="flex items-center">
                        <input type="radio" name="is_manual_selection" value="0" id="auto_selection"  {{old('is_manual_selection',$cashdraw->is_manual_selection) == 0 ? 'checked' : ''}}
                    ...
                    <div class="flex items-center">
                        <input type="radio" name="is_manual_selection" value="1" id="manual_selection" {{old('is_manual_selection',$cashdraw->is_manual_selection) == 1 ? 'checked' : ''}}
                            class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300">
            ...
            <!-- status select option-->
             <div class="mt-4">
                ...
                <select name="status" id="status" required
                    ...
                    <option value="active" {{old('status',$cashdraw->status) == 'active' ? 'selected' : ''}}>Active</option>
                    <option value="inactive" {{old('status',$cashdraw->status) == 'inactive' ? 'selected' : ''}}>Inactive</option>
                    <option value="completed" {{old('status',$cashdraw->status) == 'completed' ? 'selected' : ''}}>Completed</option>
                </select>
                @error('status')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
